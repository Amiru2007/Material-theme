...
(function bloom() {
  ...
  waitForElements(['.main-yourLibraryX-navItem'], () => {
    const navItems = document.getElementsByClassName('main-yourLibraryX-navItem'); // Targeting li elements directly
    Array.from(navItems).forEach((navItem) => {
      ...
      if (anchorTag) {
        ...
        const navItemAccentText = document.createElement('p'); // Using <p> tag
        navItemAccentText.classList.add('main-yourLibraryX-navLink-accent-text');
        
        // Retrieve the aria-label value from the <a> tag inside the navItem
        const ariaLabelValue = anchorTag.getAttribute('aria-label');
        console.log('aria-label:', ariaLabelValue); // Log the aria-label for debugging
        navItemAccentText.textContent = ariaLabelValue ? ariaLabelValue : 'No aria-label found';
        
        // Append the div and paragraph elements to the anchor tag
        anchorTag.appendChild(navItemAccent);
        anchorTag.appendChild(navItemAccentText);
  
        // Log the parent and child elements to the console for debugging
        console.log('navItem:', navItem);
        console.log('navItemAccent:', navItemAccent);
        console.log('navItemAccentText:', navItemAccentText);
      } else {
        console.log('No <a> tag found inside navItem');
      }
    });
  });
  ...
  // Function to add accent divs to navigation items
  function addAccentDivs() {
    const navItems = document.getElementsByClassName('marketplace-tabBar-headerItemLink');
    Array.from(navItems).forEach((navItem) => {
      if (!navItem.querySelector('.marketplace-tabBar-headerItemLink-accent')) {
        const navItemAccent = document.createElement('div');
        navItemAccent.classList.add('marketplace-tabBar-headerItemLink-accent');
        navItem.appendChild(navItemAccent);
      }
    });
  }
  
  // Initial execution when the app loads
  addAccentDivs();
  ...
  document.addEventListener("DOMContentLoaded", function() {
    ...
    function isNavbarExpanded() {
        return navbar.classList.contains('expanded');
    }

    // Function to toggle the state of the navbar
    function toggleNavbar() {
        navbar.classList.toggle('expanded');
        // Triggering class attribute change manually for demonstration purposes
        triggerClassChange();
    }

    // Observer callback function
    function handleNavbarChanges(mutationsList, observer) {
        for(let mutation of mutationsList) {
            if (mutation.type === 'attributes' && mutation.attributeName === 'class') {
                if (isNavbarExpanded()) {
                    console.log('Nav bar is expanded');
                } else {
                    console.log('Nav bar is collapsed');
                }
            }
        }
    }
    ...
    const observer = new MutationObserver(handleNavbarChanges);
    const config = { attributes: true, attributeFilter: ['class'] };
    observer.observe(navbar, config);

    // Function to trigger class attribute change manually for demonstration purposes
    function triggerClassChange() {
        const event = new Event('DOMAttrModified');
  ...
  const interval = setInterval(() => {
    ...
    playButtonStyle.innerHTML = `
      ...
      .main-playButton-PlayButton>button[aria-label*="${playlistPlayLabel}"] {
        ...
        -webkit-mask-image: url('https://raw.githubusercontent.com/Amiru2007/Material-theme/main/assests/icons/play.svg') !important;
      }
      ...
      .main-playButton-PlayButton>button[aria-label*="${playlistPauseLabel}"] {
        background-color: var(--spice-text) !important;
        -webkit-mask-image: url('https://raw.githubusercontent.com/Amiru2007/Material-theme/main/assests/icons/pause.svg') !important;
      }
...
document.addEventListener('DOMContentLoaded', function() {
  const sidebarToggleButton = document.querySelector('.main-yourLibraryX-collapseButtonWrapper button');
  ...
  function applyDebuggingStyle(isExpanded) {
      navLinks.forEach(link => {
          if (isExpanded) {
              link.style.border = '2px solid red';  // Example debugging style
          } else {
              link.style.border = '';  // Reset debugging style
          }
      });
  }

  function handleSidebarToggle() {
      const isExpanded = sidebarToggleButton.getAttribute('aria-label') === 'Collapse Your Library';
      applyDebuggingStyle(isExpanded);
  }
